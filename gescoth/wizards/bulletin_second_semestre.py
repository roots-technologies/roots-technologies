    ...
    def imprimer_bulletin_second_semestre(self):
        ...
        for el in eleve_ids:
            ...
            el_note_ids = self.env['gescoth.note'].search([('eleve_id', '=', el.id), (
                'saison', '=', 's2'), ('annee_scolaire', '=', self.annee_scolaire_id.id)])
        ...
        for el in liste_note:
            moyenne_annuel = (el['premier_semestre']
                              ['moyenne'] + el['moyenne_sur_vingt'])/2
            toute_les_note_annuelle.append(round(moyenne_annuel, 2))

        for el in liste_note:
            moyenne_annuel = round(
                ((el['premier_semestre']['moyenne'] + el['moyenne_sur_vingt'])/2), 2)
