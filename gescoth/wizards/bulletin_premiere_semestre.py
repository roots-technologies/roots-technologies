...
class GescothBulletinPremierSemestre(models.TransientModel):
    _name = 'gescoth.bulletin.premier.semestre'
    _description = "Impression des bulltins du premier semestre"
    ...
    @api.multi
    def imprimer_bulletin_premier_semestre(self):
        data = {}
        liste_note = []
        liste_note_total = []
        ...
        for el in eleve_ids:
            ...
            el_note_ids = self.env['gescoth.note'].search([('eleve_id', '=', el.id), (
                'saison', '=', 's1'), ('annee_scolaire', '=', self.annee_scolaire_id.id)])
        ...
        for el in liste_note:
            ...
            template = "bulltin_premiere_semestre_report_view"
        return self.env.ref('gescoth.' + template).report_action(self, data=data)
