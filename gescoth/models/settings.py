...
class GescothSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].set_param(
            'gescoth.chef_etablissement', self.chef_etablissement.id)
        self.env['ir.config_parameter'].set_param(
            'gescoth.annee_scolaire_id', self.annee_scolaire_id.id)
        self.env['ir.config_parameter'].set_param('gescoth.ville', self.ville)
        self.env['ir.config_parameter'].set_param(
            'gescoth.entete', self.entete)
        self.env['ir.config_parameter'].set_param(
            'gescoth.activate', self.cle_activation)
        self.env['ir.config_parameter'].set_param(
    ...
    def get_values(self):
        ...
        _chef_etablissement = int(
            ICPSudo.get_param('gescoth.chef_etablissement'))
        _annee_scolaire_id = int(
            ICPSudo.get_param('gescoth.annee_scolaire_id'))
