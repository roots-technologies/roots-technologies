...
class GescothPayementEleve(models.Model):
    ...
    reste_a_payer = fields.Float(
        string="Reste à payer", compute="onchange_montant")
    ...
    def _onchange_eleve_id(self):
        for rec in self:
            ...
            rec.total_a_payer = rec.eleve_id.classe.niveau_id.frais_total
            rec.reste_a_payer = rec.total_a_payer - rec.eleve_id.paiement_total
...
class GescothTranche(models.Model):
    ...
    date_premier_tranche = fields.Date(
        string="Date de la première tranche", required=True, track_visibility='always',)
    line_ids = fields.One2many(
        'gescoth.tranche.line', 'tranche_id', string="Linge de tranche")
    ...
    def calculer_tranche(self):
        ...
        traches = self.env['gescoth.tranche.line'].search(
            [('tranche_id', '=', self.id)])
        for t in traches:
            t.unlink()
        ...
        for n in range(0, self.nombre):
            ...
            my_date = date(my_date.year, my_date.month,
                           self.date_premier_tranche.day)
...
class GescothPaiementLine(models.Model):
    ...
    def Payer_la_trache(self):
        for rec in self:
            vals = {
                'eleve_id': rec.tranche_id.eleve_id.id,
                'date_paiement': fields.date.today(),
                'montant': rec.montant,
                'classe_id': rec.tranche_id.eleve_id.classe.id,
                'annee_scolaire_id': rec.tranche_id.annee_scolaire_id.id,
            }
            if rec.paye:
...
class GescothProfPaiement(models.Model):
    ...
    def onchange_professeur_id(self):
        for rec in self:
            ...
            rec.montant_par_heure = rec.professeur_id.paie_par_heure
