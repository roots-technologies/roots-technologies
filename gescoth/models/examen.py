...
class GescothCoeficient(models.Model):
    ...
    matiere = fields.Many2one(
        'gescoth.matiere', string="Matière", required=True)
    ...
    sequence = fields.Integer(
        'sequence', help="Sequence for the handle.", default=10)
...
class GesocthNote(models.Model):
    ...
    _rec_name = 'eleve_id'
    _sql_constraints = [
        ('saison_unique_note', 'UNIQUE (eleve_id, classe_id, coeficient_id, saison)',
    ...
    coeficient_id = fields.Many2one(
        'gescoth.coeficient', string="Matière", required=True)
    saison = fields.Selection(
        [('s1', 'Semestre 1'), ('s2', 'Semestre 2'), ('s3', 'Semestre 3')], required=True)
    ...
    annee_scolaire = fields.Many2one(
        'gescoth.anneescolaire', required=True, string="Année scolaire",)
    professeur_id = fields.Many2one('gescoth.professeur', "Professeur")
    ...
    def check_notes(self):
        for rec in self:
            ...
            if rec.note_compo < 0 or rec.note_compo > 20:
                raise ValidationError(
                    _('La moyenne de classe doit être entre 0 et 20. Vous avez taper : ' + str(rec.note_compo)))
